...
export interface ImageFilters {
  /** `default: 0` */
  exposure?: number;
  /** `default: 0` Value from -0.3 to 0.3*/
  contrast?: number;
  /** `default: 0` */
  saturation?: number;
  /** `default: 0` */
...
/** The root node */
export type DOCUMENT = NodeCommon & {
  type: "DOCUMENT";
...
export interface FRAME extends StrokesMixin, LayoutMixin {
  ...
  /** Max width. This property is only applicable for auto-layout frames. */
  maxWidth?: number;
  /** Min width. This property is only applicable for auto-layout frames. */
  minWidth?: number;
  /** Max height. This property is only applicable for auto-layout frames. */
  maxHeight?: number;
  /** Min height. This property is only applicable for auto-layout frames. */
  minHeight?: number;
...
export interface VECTOR extends StrokesMixin, LayoutMixin {
  /** default: [] An array of export settings representing images to export from node */
  exportSettings: ExportSetting[];
  /** If true, layer is locked and cannot be edited, default `false` */
  locked?: boolean;
...
export type Node = NodeCommon &
  (
    ...
    | ({ type: "BOOLEAN_OPERATION" } & BOOLEAN_OPERATION)
    | ({ type: "STAR" } & STAR)
    | ({ type: "LINE" } & LINE)
    | ({ type: "ELLIPSE" } & ELLIPSE)
    | ({ type: "REGULAR_POLYGON" } & REGULAR_POLYGON)
    | ({ type: "RECTANGLE" } & RECTANGLE)
    | ({ type: "TEXT" } & TEXT)
    | ({ type: "SLICE" } & SLICE)
    | ({ type: "COMPONENT" } & COMPONENT)
    | ({ type: "COMPONENT_SET" } & COMPONENT_SET)
